...
import { getAdminEvents } from "@/server/services/events.service";
...
export default async function AdminEventsPage() {
  ...
  const [events, mediaRows] = await Promise.all([getAdminEvents(), canWrite ? getMediaPickerItems() : Promise.resolve([])]);
  const mediaOptions = mediaRows.map((item) => ({ id: item.id, label: item.alt || item.originalFilename, publicUrl: item.publicUrl }));
  return <AdminEventsManager events={events} mediaOptions={mediaOptions} canWrite={canWrite} canPublish={hasPermission(currentUser.role, "content:publish")} />;
}
