  ...
  return <>
    ...
    <Card className="admin-settings-card admin-settings-form-card">
      <Form form={form} layout="vertical" initialValues={initialSettings} onFinish={submit} requiredMark="optional" scrollToFirstError>
        ...
        <div className="admin-form-grid"><Form.Item label="Avatar"><AdminMediaPicker items={mediaOptions} value={avatarMediaId ?? undefined} label="Avatar" purpose="avatar" onChange={(selection) => form.setFieldValue("avatarMediaId", selection?.id ?? null)} /></Form.Item><Form.Item label="Cover / social image"><AdminMediaPicker items={mediaOptions} value={coverMediaId ?? undefined} label="Cover" purpose="site_banner" onChange={(selection) => form.setFieldValue("coverMediaId", selection?.id ?? null)} /></Form.Item></div>
