...
import type { AdminEvent, AdminEventListQuery, AdminListPage, EventMutationInput, MediaOption } from "@/types";

interface Props { initialPage: AdminListPage<AdminEvent, AdminEventListQuery["sortBy"]>; mediaOptions: readonly MediaOption[]; canWrite: boolean; canPublish: boolean; }
...
export function AdminEventsManager({ initialPage, mediaOptions, canWrite, canPublish }: Props) {
  ...
  const { data, load, loading, reload } = useAdminListPage("/api/admin/events", initialPage, reportListError);
  ...
  const archive = (event: AdminEvent) => modal.confirm({ title: `Lưu trữ “${event.title}”?`, okText: "Lưu trữ", cancelText: "Hủy", okButtonProps: { danger: true }, onOk: async () => { const result = await archiveEventAction(event.id); if (!result.success) throw new Error(result.message); void message.success(result.message); await reload(); router.refresh(); } });
  ...
  return <>
    ...
    <section className="admin-panel admin-table-panel" aria-label="Danh sách sự kiện"><Table rowKey="id" columns={columns} dataSource={[...data.items]} loading={pending || loading} scroll={{ x: 900 }} pagination={{ ...adminTablePaginationDefaults, current: data.page, pageSize: data.pageSize, total: data.total }} onChange={(pagination) => void load({ page: pagination.current ?? 1, pageSize: pagination.pageSize ?? data.pageSize, sortBy: data.sortBy, sortOrder: data.sortOrder })} locale={{ emptyText: "Chưa có sự kiện." }} /></section>
